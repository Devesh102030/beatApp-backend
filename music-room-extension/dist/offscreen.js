...
async function request(baseUrl, path, options, label) {
  ...
  if (!res.ok) {
    ...
    const message = backendMsg ? `${backendMsg}${backendCode ? ` (${backendCode})` : ""}` : `${label ?? path} failed with status ${res.status}`;
...
const api = {
  getLiveKitToken(baseUrl, roomCode, userId, hostSecret) {
    return request(
      baseUrl,
      "/livekit/token",
      {
  ...
  broadcastStarting(baseUrl, roomCode, body) {
    return request(
      baseUrl,
  ...
  broadcastLive(baseUrl, roomCode, hostSecret) {
    return request(
      baseUrl,
  ...
  broadcastStopped(baseUrl, roomCode, hostSecret) {
    return request(
      baseUrl,
...
var firefoxShim = /* @__PURE__ */ Object.freeze({ __proto__: null, shimAddTransceiver, shimCreateAnswer, shimCreateOffer, shimGetDisplayMedia, shimGetParameters, shimGetStats, shimGetUserMedia: shimGetUserMedia$1, shimOnTrack, shimPeerConnection, shimRTCDataChannel, shimReceiverGetStats, shimRemoveStream, shimSenderGetStats });
...
var sdp = /* @__PURE__ */ _mergeNamespaces$1({ __proto__: null, default: SDPUtils }, [sdpExports]);
...
function getNewAudioContext() {
  ...
  if (AudioContext2) {
    ...
    if (audioContext2.state === "suspended" && typeof window !== "undefined" && ((_a2 = window.document) === null || _a2 === void 0 ? void 0 : _a2.body)) {
      const handleResume = () => __awaiter(this, void 0, void 0, function* () {
        var _a3;
        try {
          if (audioContext2.state === "suspended") {
            yield audioContext2.resume();
          }
        } catch (e2) {
      ...
      audioContext2.addEventListener("statechange", () => {
        var _a3;
        if (audioContext2.state === "closed") {
          (_a3 = window.document.body) === null || _a3 === void 0 ? void 0 : _a3.removeEventListener("click", handleResume);
        }
      });
...
const U16_MAX_SIZE = 65535;
const U32_MAX_SIZE = 4294967295;
class WrapAroundUnsignedInt {
  static u16(raw2) {
    return new WrapAroundUnsignedInt(raw2, U16_MAX_SIZE);
  }
  static u32(raw2) {
    return new WrapAroundUnsignedInt(raw2, U32_MAX_SIZE);
  }
...
class DataTrackClock {
  ...
  static startingNow(base2, rateInHz) {
    return new DataTrackClock(rateInHz, /* @__PURE__ */ new Date(), base2);
  }
  static startingAtTime(epoch, base2, rateInHz) {
    return new DataTrackClock(rateInHz, epoch, base2);
  }
  static rtpStartingNow(base2) {
    return DataTrackClock.startingNow(base2, 9e4);
  }
  static rtpStartingAtTime(epoch, base2) {
    return DataTrackClock.startingAtTime(epoch, base2, 9e4);
  }
  now() {
  ...
  close() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (this.publisher && this.publisher.getSignallingState() !== "closed") {
        const publisher2 = this.publisher;
        for (const sender of publisher2.getSenders()) {
          try {
            if (publisher2.canRemoveTrack()) {
              publisher2.removeTrack(sender);
            }
          } catch (e2) {
...
class LocalAudioTrack extends LocalTrack {
  ...
  constructor(mediaTrack, constraints) {
    ...
    let audioContext2 = arguments.length > 3 ? arguments[3] : void 0;
    let loggerOptions = arguments.length > 4 ? arguments[4] : void 0;
...
class RegionUrlProvider {
  ...
  updateToken(token) {
    ...
    RegionUrlProvider.scheduleRefetch(this.serverUrl, this.token, (_a2 = settings === null || settings === void 0 ? void 0 : settings.maxAgeInMs) !== null && _a2 !== void 0 ? _a2 : DEFAULT_MAX_AGE_MS);
  }
  isCloud() {
...
const VERSION_MASK = 7;
const FRAME_MARKER_SHIFT = 3;
const FRAME_MARKER_MASK = 3;
const FRAME_MARKER_START = 2;
const FRAME_MARKER_FINAL = 1;
const FRAME_MARKER_INTER = 0;
const FRAME_MARKER_SINGLE = 3;
const EXT_WORDS_INDICATOR_SIZE = 2;
const EXT_FLAG_SHIFT = 2;
const EXT_FLAG_MASK = 1;
const EXT_TAG_PADDING = 0;
...
class DataTrackDeserializeError extends LivekitReasonedError {
  ...
  static unsupportedVersion(version2) {
    return new DataTrackDeserializeError("Unsupported version ".concat(version2), DataTrackDeserializeErrorReason.UnsupportedVersion);
  }
...
const DataTrackSymbol = Symbol.for("lk.data-track");
...
const SUBSCRIBE_TIMEOUT_MILLISECONDS = 1e4;
...
const PUBLISH_TIMEOUT_MILLISECONDS = 1e4;
...
class RemoteAudioTrack extends RemoteTrack {
  constructor(mediaTrack, sid, receiver, audioContext2, audioOutput, loggerOptions) {
  ...
  setAudioContext(audioContext2) {
    this.audioContext = audioContext2;
    if (audioContext2 && this.attachedElements.length > 0) {
      this.connectWebAudio(audioContext2, this.attachedElements[0]);
    } else if (!audioContext2) {
      this.disconnectWebAudio();
    }
  }
  /**
        ...
        try {
          ...
          if (track.sender && ((_a3 = this.engine.pcManager) === null || _a3 === void 0 ? void 0 : _a3.publisher)) {
            try {
              ...
              this.log.error(e2, this.logContext);
            }
      ...
      if (this.engine.pcManager && this.engine.pcManager.currentState < PCTransportState.FAILED && trackSender) {
        try {
          ...
          try {
            ...
            this.log.warn(e2, this.logContext);
            negotiationNeeded = true;
          }
          if (isLocalVideoTrack(track)) {
  ...
  onTrackAdded(mediaTrack, stream, receiver) {
    if (this.state === ConnectionState.Connecting || this.state === ConnectionState.Reconnecting) {
      ...
      const cleanup2 = () => {
        ...
        this.off(RoomEvent.Disconnected, cleanup2);
      };
      this.once(RoomEvent.Reconnected, reconnectedHandler);
      this.once(RoomEvent.Connected, reconnectedHandler);
      this.once(RoomEvent.Disconnected, cleanup2);
      return;
    }
    if (this.state === ConnectionState.Disconnected) {
...
Room.cleanupRegistry = typeof FinalizationRegistry !== "undefined" && new FinalizationRegistry((cleanup2) => {
  cleanup2();
});
...
class LiveKitPublisher {
  constructor() {
    ...
    __publicField(this, "localTrack", null);
  }
  async connect(options) {
    ...
    const localTrack = new LocalAudioTrack(options.audioTrack, void 0, false);
    this.localTrack = localTrack;
    await room.localParticipant.publishTrack(localTrack, {
      name: "host-tab-audio",
      source: Track.Source.Microphone
      // closest semantic match for tab audio
    });
    this.room = room;
  }
  async disconnect() {
...
function sendStatus(status, error) {
  const msg = { type: "BROADCAST_STATUS", status, error };
  chrome.runtime.sendMessage(msg).catch(() => {
  });
}
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "START_BROADCAST") {
    handleStart(message).then(() => sendResponse({ ok: true })).catch((err) => sendResponse({ ok: false, error: String(err) }));
    return true;
  }
  if (message.type === "STOP_BROADCAST") {
    handleStop().then(() => sendResponse({ ok: true })).catch(() => sendResponse({ ok: false }));
    return true;
  }
});
...
async function captureTabAudioFromStreamId(streamId) {
...
function preserveLocalPlayback(stream) {
  try {
